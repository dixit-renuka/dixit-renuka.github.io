...
class Pulse {
  ...
  draw(ctx) {
    ...
    g.addColorStop(0,   this.pal.pulse.replace('{a}', '0.25'));
    g.addColorStop(0.4, this.pal.pulse.replace('{a}', '0.08'));
...
class NeuralNode {
  constructor(w, h) {
    ...
    const speed  = (0.02 + Math.random() * 0.06) * (0.4 + this.depth * 0.6);
  ...
  draw(ctx) {
   ...
   const alpha  = (0.4 + this.depth * 0.6) * breath;
    ...
    grd.addColorStop(0,   this.pal.glow.replace('{a}', (alpha * 0.28).toFixed(2)));
    grd.addColorStop(0.5, this.pal.glow.replace('{a}', (alpha * 0.07).toFixed(2)));
    ...
    ctx.fillStyle   = this.pal.core;
    ctx.shadowColor = this.pal.core;
    ctx.shadowBlur  = this.type === 'soma' ? 6 : 3;
...
function drawSynapse(ctx, a, b, dist, maxDist) {
  ...
  ctx.lineWidth   = 0.4 + (1 - dist / maxDist) * 0.3;
  ctx.stroke();
}
...
// ── PROJECT CARD FILTER ────────────────
document.querySelectorAll('.tl-btn[data-ptab]').forEach(btn => {
  btn.addEventListener('click', () => {
    document.querySelectorAll('.tl-btn[data-ptab]').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    const tab = btn.dataset.ptab;
    document.querySelectorAll('.proj-card').forEach(card => {
      card.classList.toggle('hidden', tab !== 'all' && card.dataset.ptype !== tab);
    });
  });
});
...
document.querySelectorAll('.proj-card[data-pid]').forEach(card => {
  card.addEventListener('click', () => openPanel(card.dataset.pid));
});
